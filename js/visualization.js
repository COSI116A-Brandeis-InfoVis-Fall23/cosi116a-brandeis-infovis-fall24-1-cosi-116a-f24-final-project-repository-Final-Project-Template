document.addEventListener("DOMContentLoaded", function () {
        ...
        .await(function (error, us, csvData) {
            ...
            csvData.forEach(d => {
                ...
                dataMap[fips][d.Year] = {
                    ...
                    income: +d["Median Income"]  // Store income as a number, not a percentage
                };
            });
            ...
            function updateRankingTable(selectedYear, selectedMetric) {
                const tableContainer = d3.select("#ranking-table-container");
                ...
                for (let fips in dataMap) {
                    if (dataMap[fips][selectedYear] && dataMap[fips][selectedYear][selectedMetric] != null) {
                        rankings.push({
                            state: fipsToStateName[fips],
                            value: dataMap[fips][selectedYear][selectedMetric]
                        });
                    }
                }
                ...
                const nationalAverage = d3.mean(rankings, d => d.value);
                ...
                table.append("thead").html(`
                        <tr>
                            <th>Rank</th>
                            <th>State</th>
                            <th>${selectedMetric.toUpperCase()} Value</th>
                ...
                function formatValue(value, metric) {
                    if (metric === 'income') {
                        ...
                        return `${(value * 100).toFixed(1)}%`; // Format as percentage
                    }
                }
                ...
                top3.forEach((d, i) => {
                    tbody.append("tr").html(`
                            <td>${i + 1}</td>
                            <td>${d.state}</td>
                            <td>${formatValue(d.value, selectedMetric)}</td>
                            <td class="${d.value >= nationalAverage ? 'positive' : 'negative'}">
                                ${d.value >= nationalAverage ? '+' : ''}${formatValue(d.value - nationalAverage, selectedMetric)}
                            </td>
                        `);
                });
            ...
            function updateMap() {
                ...
                updateRankingTable(selectedYear, selectedMetric);
            }
    ...
    function renderBarAndLineCharts() {
        ...
        d3.csv(dataPath, function (error, rawData) {
            ...
            // Draw bars
            ['Mortgage', 'Rent', 'INCOME'].forEach((key, i) => {
                barSvg.selectAll(`.bar-${key}`)
                    .data(data)
                    .enter().append("rect")
                    .attr("class", `bar-${key}`)
                    .attr("x", d => xBar(d.Year) + (i * xBar.bandwidth() / 3))
                    .attr("y", d => yBar(d[key]))
                    .attr("width", xBar.bandwidth() / 3)
                ...
                // barSvg.selectAll(`.label-${key}`)
                //     .data(data)
                //     .enter().append("text")
                //     .attr("class", `label-${key}`)
                //     .attr("x", d => xBar(d.Year) + (i * xBar.bandwidth() / 3) + (xBar.bandwidth() / 6))
                //     .attr("y", d => yBar(d[key]) - 5)
                //     .attr("text-anchor", "middle")
                //     .text(d => key === 'INCOME' ? d[key] : `${d[key]}%`);
            });
